...
jail.setSync('fetch', function(url) {
    fetch(url).then(response => response.json()).then(data => console.log(data));
});

jail.setSync('require', function(module) {
    console.log(module);
})

jail.setSync('readFileSync', function(path, encoding) {
...
// Read webpack code
isolate.compileScriptSync(fs.readFileSync('./dist/main.js', 'utf8')).runSync(context)

const hostile = isolate.compileScriptSync(`
    const file_data = readFileSync('./package.json', 'utf8');
    log(file_data);
